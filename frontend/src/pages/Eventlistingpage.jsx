...
import { Calendar, Users, MapPin, Clock, Search, Filter, X, ChevronDown, ArrowUpDown, User,Home,DollarSign } from 'lucide-react';
...
const EventsListing = () => {
  ...
  const fetchEvents=async()=>{
    try{
      ...
      if (sortBy) params.append('sort', sortBy === 'latest' ? 'createdAt' : sortBy === 'price-low' ? 'price' : 'createdAt');

      const response = await fetch(`${API_ENDPOINTS.GET_ALL_EVENTS}?${params}`);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-r from-purple-600 to-blue-600 text-white py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-purple-100 text-lg">Find and register for amazing events near you</p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl shadow-md p-6 mb-8">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value)}
              className="px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-600 outline-none"
            >
              <option value="latest">Latest</option>
            ...
            {filteredEvents.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                {filteredEvents.map((event) => (
                  <div
                    key={event._id}
                    className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition cursor-pointer transform hover:scale-105"
                    onClick={() => navigate(`/events/${event._id}`)}
                  >
                    <div className="relative h-48">
                      <img
                        src={event.imageUrl || 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=400&h=250&fit=crop'}
                        alt={event.title}
                        className="w-full h-full object-cover"
                      />
                      <div className="absolute top-3 right-3 px-3 py-1 bg-white rounded-full text-sm font-semibold text-purple-600">
                        {event.category}
                      </div>
                    </div>
